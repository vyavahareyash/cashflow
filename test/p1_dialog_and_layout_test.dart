...
void main() {
  group('Issue #67: AppDialogs.showWarning validation dialogs', () {
    testWidgets(
      'AppDialogs.showWarning renders warning dialog with title, message, icon and OK button',
      (tester) async {
    ...
    testWidgets('AppDialogs.showWarning defaults to Validation Error title', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
  ...
  group('Issue #71: EditAccountDialog action buttons horizontal layout', () {
    testWidgets(
      'Edit Account dialog has header delete icon and single horizontal actions row',
      (tester) async {
    ...
    testWidgets(
      'Edit Credit Card dialog has header delete icon and actions with Cancel and Update',
      (tester) async {
  ...
  group(
    'Issue #67: TransferFundsModal validation triggers AppDialogs.showWarning',
    () {
      testWidgets(
        'Transfer validation shows dialog pop-up instead of SnackBar',
        (tester) async {
          ...
          // Modal is now open
          expect(
            find.textContaining('Transfer from "Account A"'),
            findsOneWidget,
          );

          final completeBtn = find.widgetWithText(
            ElevatedButton,
            'Complete Transfer',
          );
          ...
          expect(find.byType(AlertDialog), findsOneWidget);
          expect(
            find.textContaining('Cannot transfer more than account balance'),
            findsOneWidget,
          );
          expect(find.byType(SnackBar), findsNothing);
