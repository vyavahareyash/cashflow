        ...
        body: Builder(
          builder: (context) => Center(
            child: ElevatedButton(
              ...
              onPressed: () =>
                  VoiceRecordingModal.show(context, coordinator: testCoord),
  ...
  group('VoiceRecordingModal Widget & A11y Tests (US 1, 3, 13, 16)', () {
    Future<void> waitForRecordingReady(WidgetTester tester) async {
      for (int i = 0; i < 50; i++) {
        ...
        if (find
                .byKey(const Key('voice_recording_done_button'))
                .evaluate()
                .isNotEmpty ||
            find
    ...
    testWidgets(
      '1. Renders listening UI with timer, mic button, live transcript card, and controls',
      (tester) async {
        ...
        expect(
          find.byKey(const Key('voice_recording_mic_button')),
          findsOneWidget,
        );
        expect(
          find.byKey(const Key('voice_recording_live_transcript_card')),
          findsOneWidget,
        );
        expect(
          find.byKey(const Key('voice_recording_live_transcript_text')),
          findsOneWidget,
        );
        expect(find.textContaining('rupees'), findsWidgets);
        expect(
          find.byKey(const Key('voice_recording_duration_text')),
          findsNothing,
        );
        expect(
        ...
        expect(
          find.byKey(const Key('voice_recording_done_button')),
          findsOneWidget,
        );
      },
    );
    ...
    testWidgets(
      '1b. Live streaming transcript updates reactively in real time',
      (tester) async {
        await tester.pumpWidget(buildTestApp(coordinator));
        await tester.tap(find.byKey(const Key('open_modal_button')));
        await tester.pump();
    ...
    testWidgets(
      '1c. Tapping central mic button toggles mic between ON and OFF states with visual indicators',
      (tester) async {
        await tester.pumpWidget(buildTestApp(coordinator));
        await tester.tap(find.byKey(const Key('open_modal_button')));
        await tester.pump();
    ...
    testWidgets(
      '1d. Android stopping listening automatically reflects in button state as MIC OFF and allows resuming',
      (tester) async {
        await tester.pumpWidget(buildTestApp(coordinator));
        await tester.tap(find.byKey(const Key('open_modal_button')));
        await tester.pump();
        ...
        (coordinator.isMicActiveListenable as ValueNotifier<bool>).value =
            false;
        await tester.pump();
        ...
        expect(find.byIcon(Icons.mic_off_rounded), findsOneWidget);
        expect(
          find.text('Microphone paused. Tap mic to resume'),
          findsOneWidget,
        );
    ...
    testWidgets(
      '1e. User can edit transcript when mic is off to correct STT errors',
      (tester) async {
        await tester.pumpWidget(buildTestApp(coordinator));
        await tester.tap(find.byKey(const Key('open_modal_button')));
        await tester.pump();
        ...
        final editField = find.byKey(
          const Key('voice_recording_live_transcript_edit_field'),
        );
        ...
        expect(
          coordinator.currentLiveTranscript,
          equals('Chai 25 rupees on UPI'),
        );
    ...
    testWidgets(
      '2. Tap Cancel cancels capture and dismisses modal cleanly (US 13)',
      (tester) async {
        await tester.pumpWidget(buildTestApp(coordinator));
        await tester.tap(find.byKey(const Key('open_modal_button')));
        await tester.pump();
        ...
        for (int i = 0; i < 20; i++) {
          ...
          await tester.pump(const Duration(milliseconds: 50));
          if (find
              .byKey(const Key('voice_recording_modal'))
              .evaluate()
              .isEmpty) {
            break;
          }
    ...
    testWidgets(
      '3. Tap Done processes audio and transitions to staging sheet (US 1, 3)',
      (tester) async {
...
''';
    ...
    testWidgets('4. Silent audio displays friendly error message', (
      tester,
    ) async {
      await mockSttEngine.initialize(modelDirPath: modelDir.path);
      ...
      expect(
        find.byKey(const Key('voice_recording_error_text')),
        findsOneWidget,
      );
      expect(
        find.textContaining('No decipherable speech detected'),
        findsOneWidget,
      );
      expect(
        find.byKey(const Key('voice_recording_error_close_button')),
        findsOneWidget,
      );
    });

    testWidgets('5. Permission denial displays error view and close button', (
      tester,
    ) async {
    ...
    testWidgets(
      '6. Accessibility: meets WCAG AA tap targets and contrast guidelines',
      (tester) async {
    ...
    testWidgets('7. Empty drafts displays dark mode readable SnackBar', (
      tester,
    ) async {
      await mockSttEngine.initialize(modelDirPath: modelDir.path);
