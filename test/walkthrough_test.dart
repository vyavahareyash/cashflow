...
class FakeBillingService extends ChangeNotifier implements BillingService {
  ...
  @override
  void setTestingState({bool? isAvailable, bool? isLoading, List<ProductDetails>? products, String? errorMessage}) {}
}
...
void main() {
  ...
  setUp(() async {
    ...
    tempDir = await Directory.systemTemp.createTemp('cashflow_walkthrough_test_');
  ...
  group('Walkthrough DatabaseHelper Operations', () {
    test('getWalkthroughCompleted defaults to false and persists updates', () async {
    ...
    test('enterWalkthroughDemoMode snapshots user data and exitWalkthroughDemoMode restores it', () async {
      ...
      final userAccId = await db.createAccount(
        Account(name: 'My Personal Real Account', balance: 42000.0, type: 'Savings'),
      );
      final preAccounts = await db.readAllAccounts();
      expect(preAccounts.any((a) => a.id == userAccId), isTrue);
      expect(preAccounts.any((a) => a.name == 'My Personal Real Account'), isTrue);
      ...
      expect(demoAccounts.any((a) => a.name.contains('Salary Account (HDFC)')), isTrue);
      expect(demoAccounts.any((a) => a.name == 'My Personal Real Account'), isFalse);
      ...
      expect(restoredAccounts.any((a) => a.name == 'My Personal Real Account'), isTrue);
      expect(restoredAccounts.any((a) => a.name.contains('Salary Account (HDFC)')), isFalse);
    });
    ...
    test('recoverWalkthroughDemoModeIfNeeded restores snapshot if app restarted mid-demo', () async {
      ...
      expect(accounts.any((a) => a.name.contains('Salary Account (HDFC)')), isFalse);
    });
  });

  group('WalkthroughConceptCarousel Widget Tests', () {
    testWidgets('renders all 5 concept slides and allows forward/backward paging', (tester) async {
      ...
      expect(find.byKey(const Key('walkthrough_skip_button')), findsOneWidget);
      ...
      expect(find.byKey(const Key('walkthrough_explore_button')), findsOneWidget);
      expect(find.byKey(const Key('walkthrough_take_tour_button')), findsOneWidget);
  ...
  group('WalkthroughSpotlightOverlay Widget Tests', () {
    testWidgets('renders demo banner and navigates through steps', (tester) async {
      int currentStep = 0;
      ...
      expect(find.byKey(const Key('walkthrough_tour_finish_button')), findsOneWidget);
  ...
  group('Settings & Data Screen Walkthrough Section', () {
    testWidgets('renders App Walkthrough & Concepts section with 3 tiles', (tester) async {
      ...
      // 3 Tiles
      expect(find.byKey(const Key('settings_full_walkthrough_tile')), findsOneWidget);
      expect(find.byKey(const Key('settings_review_concepts_tile')), findsOneWidget);
      expect(find.byKey(const Key('settings_screen_tour_tile')), findsOneWidget);
    });
  });

  group('MainNavigationScreen Walkthrough First-Install & Tour Flow', () {
    testWidgets('automatically prompts concept carousel on first install and launches spotlight tour', (tester) async {
      tester.view.physicalSize = const Size(1080, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      expect(find.text('Walkthrough Demo Mode — Your data is preserved'), findsOneWidget);
      expect(DatabaseHelper.instance.isWalkthroughDemoMode, isTrue);

      // Step through all spotlight steps
      for (int i = 0; i < 4; i++) {
        expect(find.byKey(const Key('walkthrough_tour_next_button')), findsOneWidget);
        await tester.tap(find.byKey(const Key('walkthrough_tour_next_button')));
        await tester.pump();
        await tester.pump(const Duration(milliseconds: 100));
      }
      ...
      expect(find.text('Walkthrough Demo Mode — Your data is preserved'), findsNothing);
      expect(DatabaseHelper.instance.isWalkthroughDemoMode, isFalse);

      bool finalCompleted = false;
      await tester.runAsync(() async {
        finalCompleted = await DatabaseHelper.instance.getWalkthroughCompleted();
      });
