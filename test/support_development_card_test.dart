...
void main() {
  ...
  const databaseFileName = 'support_card_test.db';
  ...
  group('BackupRestoreScreen Support Development Card', () {
    testWidgets(
      'Support card is hidden when both external donations and Play Store tips are false',
      (tester) async {
        AppConfig.setOverrideEnableExternalDonations(false);
        AppConfig.setOverrideEnablePlayStoreTips(false);
    ...
    testWidgets(
      'Support card displays Buy Me a Coffee button when enableExternalDonations is true',
      (tester) async {
        AppConfig.setOverrideEnableExternalDonations(true);
        AppConfig.setOverrideEnablePlayStoreTips(false);
    ...
    testWidgets(
      'Support card displays Play Store Tip Jar UI when enablePlayStoreTips is true',
      (tester) async {
        AppConfig.setOverrideEnableExternalDonations(false);
    ...
    testWidgets(
      'Support card displays product tip buttons when products are available',
      (tester) async {
        AppConfig.setOverrideEnableExternalDonations(false);
        AppConfig.setOverrideEnablePlayStoreTips(true);
        ...
        expect(
          find.byKey(const Key('tip_button_coffee_single')),
          findsOneWidget,
        );
        expect(
          find.byKey(const Key('tip_button_coffee_double')),
          findsOneWidget,
        );
        expect(
          find.text('Fuel a quick bug fix or optimization'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'Developer & Community footer displays correctly with GitHub and LinkedIn buttons',
      (tester) async {
        await tester.pumpWidget(const MaterialApp(home: BackupRestoreScreen()));
        await tester.pumpAndSettle();
        ...
        expect(
          find.byKey(const Key('developer_github_button')),
          findsOneWidget,
        );
        expect(
