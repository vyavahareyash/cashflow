...
void main() {
  ...
  group('Activity Ledger CSV Export Destination Dialog (Issue #80)', () {
    testWidgets(
      'tapping export button opens export dialog with default directory and csv filename',
      (tester) async {
        ...
        await tester.runAsync(() async {
          ...
          final catId = await db.createCategory(
            Category(name: 'Groceries', monthlyBudget: 200.0),
          );
        ...
        final exportBtn = find.byKey(
          const Key('activity_ledger_export_csv_btn'),
        );
    ...
    testWidgets(
      'canceling export dialog dismisses dialog and shows cancelled message',
      (tester) async {
        tester.view.physicalSize = const Size(1080, 2400);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() => tester.view.resetPhysicalSize());
        ...
        // Tap export button
        await tester.tap(
          find.byKey(const Key('activity_ledger_export_csv_btn')),
        );
        await tester.pump();
        await tester.runAsync(() async {
          await Future.delayed(const Duration(milliseconds: 100));
    ...
    testWidgets(
      'editing filename and confirming exports CSV and shows success message',
      (tester) async {
        tester.view.physicalSize = const Size(1080, 2400);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() => tester.view.resetPhysicalSize());
        ...
        for (int i = 0; i < 30; i++) {
          ...
          await tester.pump();
          if (find
              .textContaining('custom_ledger_export.csv')
              .evaluate()
              .isNotEmpty) {
            break;
          }
        ...
        expect(
          find.textContaining('custom_ledger_export.csv'),
