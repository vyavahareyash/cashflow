...
void main() {
  ...
  group('Goal transaction history retrieval and search', () {
    test('getGoalTransactions returns only transactions for target goal', () async {
  ...
  group('Goal transaction editing and atomic synchronization', () {
    ...
    test('editing goal_lock account shifts locked allocation across accounts', () async {
      final db = DatabaseHelper.instance;
      ...
      var a1Contrib = await db.getGoalContributions(goalId);
    ...
    test('editing goal_lock beyond available usable balance throws error', () async {
      final db = DatabaseHelper.instance;
      final accId = await db.createAccount(
    ...
    test('editing goal_payment synchronizes account balance and locked funds', () async {
      final db = DatabaseHelper.instance;
      final accId = await db.createAccount(
        Account(name: 'Checking', balance: 2000.0, type: 'Bank'),
      );
  ...
  group('Goal transaction deletion rollback', () {
    ...
    test('deleting goal_payment restores locked allocation and refunds account', () async {
      final db = DatabaseHelper.instance;
      final accId = await db.createAccount(
        Account(name: 'Checking', balance: 2000.0, type: 'Bank'),
      );
  ...
  group('GoalsScreen contribution log data and component tests', () {
    test('verifies data provider for goal contribution and activity log', () async {
      final db = DatabaseHelper.instance;
      final accId = await db.createAccount(
    ...
    testWidgets('renders goal contribution and activity card layout', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: Column(
              children: [
                const Text('Locked Funds by Account'),
                const Text('Contribution & Activity History'),
                const Text('Main Bank'),
                const Text('Locked Funds'),
                const Text('Savings batch 1'),
              ],
            ),
          ),
        ),
