...
void main() {
  ...
  Future<List<Map<String, dynamic>>> getLocks(DatabaseHelper db, int goalId) async {
    final rawDb = await db.database;
    return await rawDb.query('locked_allocations', where: 'goal_id = ?', whereArgs: [goalId]);
  }
  ...
  group('Transaction deletion atomic rollbacks', () {
    ...
    test('transfer deletion rolls back both source and destination accounts', () async {
      final db = DatabaseHelper.instance;
    ...
    test('goal_lock deletion reverses locked allocation and goal currentSaved', () async {
      final db = DatabaseHelper.instance;
      final accId = await db.createAccount(
        Account(name: 'Checking', balance: 500.0, type: 'Bank'),
      );
    ...
    test('goal_unlock deletion restores locked allocation and goal currentSaved', () async {
      final db = DatabaseHelper.instance;
      final accId = await db.createAccount(
        Account(name: 'Checking', balance: 500.0, type: 'Bank'),
      );
  ...
  group('Transaction update logic', () {
    test('expense update adjusts balance on amount and account change', () async {
      final db = DatabaseHelper.instance;
      ...
      var tx = (await db.getTransactionHistory()).first;
      ...
      var a2 = await getAccount(db, acc2);
      expect(a1.balance, 500.0);
      expect(a2.balance, 120.0);
    });

    test('income update adjusts balance on amount and account change', () async {
      final db = DatabaseHelper.instance;
      final acc1 = await db.createAccount(
        Account(name: 'Checking', balance: 500.0, type: 'Bank'),
      );
  ...
  group('Transaction ledger filtering and summary aggregation', () {
    ...
    testWidgets('renders period summary card with outflow, inflow, and net', (tester) async {
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Column(
                        children: [
                          const Text('Net'),
                          Text('+${AppFormatters.compactCurrency(netCashflow)}'),
                        ],
                      ),
                      Column(
                        children: [
      ...
      expect(find.byKey(const Key('activity_ledger_period_card')), findsOneWidget);
