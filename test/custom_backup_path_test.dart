...
void main() {
  ...
  group('Custom Backup Path Configuration (Issue #59)', () {
    test('persists, updates, and resets custom backup path in app_settings', () async {
    ...
    test('exportDatabase copies database to custom directory with custom filename', () async {
      final db = DatabaseHelper.instance;
    ...
    test('exportDatabaseAsJSON exports to custom directory and filename', () async {
      final db = DatabaseHelper.instance;
  ...
  group('BackupRestoreScreen Custom Location & Confirmation Flow (Issue #59)', () {
    testWidgets('renders high-level Export and Import tiles on main screen', (tester) async {
    ...
    testWidgets('shows format selection & destination confirmation dialog on export and cancels cleanly', (tester) async {
      tester.view.physicalSize = const Size(1080, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      expect(find.byKey(const Key('export_destination_directory_text')), findsOneWidget);
      expect(find.byKey(const Key('export_browse_directory_button')), findsOneWidget);
      expect(find.byKey(const Key('export_filename_input')), findsOneWidget);
      expect(find.byKey(const Key('export_full_path_preview')), findsOneWidget);
    ...
    testWidgets('shows export dialog, allows format selection and filename editing, and confirms export', (tester) async {
      tester.view.physicalSize = const Size(1080, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      expect(find.textContaining('custom_test_backup.json'), findsAtLeastNWidgets(1));
      ...
      for (int i = 0; i < 30; i++) {
        ...
        await tester.pump();
        if (find.textContaining('custom_test_backup.json').evaluate().isNotEmpty) {
          break;
        }
      }
    ...
    testWidgets('shows import dialog with format choices and cancels cleanly', (tester) async {
      tester.view.physicalSize = const Size(1080, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      expect(find.text('Select the backup file format you want to restore:'), findsOneWidget);
      expect(find.text('SQLite Database (.db)'), findsOneWidget);
      expect(find.text('JSON Backup (.json)'), findsOneWidget);
