...
void main() {
  ...
  group('DatabaseHelper App Settings & Privacy Persistence', () {
    ...
    test(
      'getPrivacyMode defaults to false and setPrivacyMode toggles it',
      () async {
        final db = DatabaseHelper.instance;
  ...
  group('Dashboard Privacy Masking & UI Formatting Tests', () {
    test('AppFormatters masks all currency types when private', () {
      ...
      expect(
        AppFormatters.compactCurrency(15000000, isPrivate: false),
        '₹1.5Cr',
      );
    ...
    testWidgets(
      'renders masked budget snapshot and goal targets when isPrivate is true',
      (tester) async {
    ...
    testWidgets(
      'renders unmasked budget snapshot and goal targets when isPrivate is false',
      (tester) async {
  ...
  group('Settings Screen Startup Privacy Preference UI Tests', () {
    testWidgets(
      'renders start in privacy mode switch and toggling persists value',
      (tester) async {
        ...
        final switchFinder = find.byKey(
          const Key('start_in_privacy_mode_switch'),
        );
        ...
        await tester.runAsync(() async {
          persistedValue = await DatabaseHelper.instance
  ...
  group('App Startup Privacy Mode Initialization Tests', () {
    testWidgets(
      'DashboardScreen launches with unmasked balances when start_in_privacy_mode is disabled',
      (tester) async {
    ...
    testWidgets(
      'DashboardScreen launches with masked balances when start_in_privacy_mode is enabled',
      (tester) async {
        DashboardScreen.resetStartupPrivacyFlag();
        tester.view.physicalSize = const Size(1080, 2400);
        tester.view.devicePixelRatio = 1.0;
    ...
    testWidgets(
      'DashboardScreen re-masks balances when resuming from background if start_in_privacy_mode is enabled',
      (tester) async {
        DashboardScreen.resetStartupPrivacyFlag();
        tester.view.physicalSize = const Size(1080, 2400);
        tester.view.devicePixelRatio = 1.0;
    ...
    testWidgets(
      'DashboardScreen preserves unmasked balances when resuming from background if start_in_privacy_mode is disabled',
      (tester) async {
        DashboardScreen.resetStartupPrivacyFlag();
        tester.view.physicalSize = const Size(1080, 2400);
        tester.view.devicePixelRatio = 1.0;
