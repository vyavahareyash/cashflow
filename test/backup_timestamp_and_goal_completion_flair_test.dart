...
void main() {
  ...
  group('Backup Freshness Timestamp Persistence (Issue #48)', () {
    test('persists and retrieves last backup timestamp via app_settings', () async {
  ...
  group('Goal Completion Celebratory Flair (Issue #48)', () {
    testWidgets('GoalCard renders celebratory flair and ready-to-settle banner when 100% completed', (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: GoalCard(goal: completedGoal),
          ),
        ),
      );
      ...
      expect(find.text('Ready to Settle • Sinking fund 100% funded'), findsOneWidget);
      expect(find.text('🎉'), findsOneWidget);
    });

    testWidgets('GoalCard does not render celebratory flair banner when active/incomplete', (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: GoalCard(goal: activeGoal),
          ),
        ),
      );
      await tester.pumpAndSettle();

      expect(find.text('Ready to Settle • Sinking fund 100% funded'), findsNothing);
