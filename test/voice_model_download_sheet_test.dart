...
void main() {
  ...
  group('VoiceModelDownloadSheet Widget Tests (US 14, US 19)', () {
    testWidgets('1. Renders title, highlights, and action buttons', (
      tester,
    ) async {
      ...
      expect(
        find.textContaining('One-Time Download (~230 MB)'),
        findsOneWidget,
      );
      ...
      // Check buttons
      expect(
        find.byKey(const Key('voice_model_download_settings_button')),
        findsOneWidget,
      );
      expect(
    ...
    testWidgets('2. Tap "Not Now" dismisses the modal bottom sheet cleanly', (
      tester,
    ) async {
      await tester.pumpWidget(buildTestApp());
      await tester.tap(find.byKey(const Key('open_sheet_button')));
    ...
    testWidgets(
      '3. Tap "Download in Settings" navigates to BackupRestoreScreen',
      (tester) async {
        await tester.pumpWidget(buildTestApp());
        await tester.tap(find.byKey(const Key('open_sheet_button')));
        await tester.pumpAndSettle();
        ...
        final backupScreen = tester.widget<BackupRestoreScreen>(
          find.byType(BackupRestoreScreen),
        );
    ...
    testWidgets(
      '4. Accessibility: meets WCAG AA tap targets and contrast guidelines',
      (tester) async {
    ...
    testWidgets(
      '5. Displays active progress and cancel button when downloading',
      (tester) async {
